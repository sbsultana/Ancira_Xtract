...
export class Dashboard {
  ...
  ngAfterViewInit(): void {
    ...
    this.excel = this.apiSrvc.getExportToExcelAllReports().subscribe((res: any) => {
      const obj = res?.obj;
      if (!obj || obj.title !== 'Enterprise Income / Expense Trend') return;
  ...
  private createPDF(): jsPDF {
    ...
    /* ================= TITLE ================= */
    doc.setFontSize(14);
    doc.text('Enterprise Income / Expense Trend', 14, 12);

    let startY = 18;

    /* ================= KEYS ================= */
    let keys = Array.isArray(this.ExpenseTrendKeys)
      ? [...this.ExpenseTrendKeys]
      : [];

    // ❌ remove BDGT like Excel
    keys = keys.filter((k: any) => !k.includes('_BDGT'));

    /* ================= HEADER ================= */
    const header = ['Department', 'Average', ...keys];
    ...
    (this.ExpenseTrendByStoreMonth || []).forEach((d: any) => {
      ...
      const isSingleRow =
        d.SequenceNo === 0 || d.CategorySequence === 0;
      ...
      if (isSingleRow) {
        ...
        const row = [
          ...
          d['Average'] ?? '-',
          ...keys.map((k: any) =>
            d[k] === 0 || d[k] == null || d[k] === '' ? '-' : Number(d[k])
          )
        ];

        body.push(row);
        rowMeta.push({ type: 'single' });
        return;
      }
      ...
      if (currentGroup !== finSummary) {
        ...
        const groupRow = [finSummary, '', ...keys.map(() => '')];
    ...
    autoTable(doc, {
      ...
      didParseCell: (data: any) => {
        ...
        if (data.column.index >= 1 && typeof data.cell.raw === 'number') {
          ...
          const rounded = Math.round(data.cell.raw);
        ...
        if (meta?.type === 'group') {
          Object.values(data.row.cells).forEach((cell: any) => {
            cell.styles.fillColor = [217, 231, 255]; // D9E7FF
        ...
        /* ===== ALTERNATE ROW ===== */
        if (meta?.type === 'normal' && data.row.index % 2 === 0) {
  ...
  generatePDF() {
    ...
    try {
      ...
      doc.save(`Enterprise Income By Expense Trend.pdf`);
    } catch (e) {
      console.error(e);
  ...
  sendEmailData(Email: any, notes: any, from: any) {
    ...
    try {
      ...
      const pdfFile = this.blobToFile(pdfBlob, 'Enterprise Income By Expense Trend.pdf');
      const formData = new FormData();
      formData.append('to_email', Email);
      formData.append('subject', 'Enterprise Income By Expense Trend');
  ...
  exportAsXLSX(): void {
    ...
    let sheetName = 'Enterprise Income By Expense Trend';
    ...
    /* ================= HEADER ================= */
    const Header = ['Department', 'Average'];
    keys.forEach((k: any) => Header.push(k));
    ...
    data.forEach((d: any) => {
      ...
      if (currentGroup !== finSummary) {
        ...
        let groupRowData = [finSummary, ''];
        keys.forEach(() => groupRowData.push(''));
    ...
    workbook.xlsx.writeBuffer().then((data: any) => {
      ...
      FileSaver.saveAs(blob, 'Enterprise Income Expense Trend.xlsx');
    });
  ...
  exportToExcel() {
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      FileSaver.saveAs(blob, `Enterprise Income By Expense Trend Details`);
    });
  }
