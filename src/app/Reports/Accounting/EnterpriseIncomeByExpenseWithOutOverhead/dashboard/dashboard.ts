...
import { EnterpriseincomebyexpensewithoutoverheadDetails } from '../enterpriseincomebyexpensewithoutoverhead-details/enterpriseincomebyexpensewithoutoverhead-details';
...
export class Dashboard {
  ...
  constructor(
    ...
    switch (this.CurrentRoute) {
      case 'FixedIncomeByExpenseWithOutOverhead':
        this.filters = ['Service', 'Parts', 'Details'];
        this.SetTitle = 'Fixed Income/Expense w/o Overhead';
        break;
      case 'VariableIncomeByExpenseWithOutOverhead':
        this.filters = ['New', 'Used'];
        this.SetTitle = 'Variable Income/Expense w/o Overhead';
        break;
      case 'EnterpriseIncomeByExpenseWithOutOverhead':
        this.filters = ['New', 'Used', 'Service', 'Parts', 'Details'];
        this.SetTitle = 'Enterprise Income/Expense w/o Overhead';
        break;
      default:
        this.filters = [];
  ...
  GetDataByMonths(date: any, filters: any) {
    ...
    this.apiSrvc
      .postmethod(this.comm.routeEndpoint + 'GetFixedIncomeExpenseWoOverhead', obj)
      .subscribe(
        (res: any) => {
  ...
  ngAfterViewInit(): void {
    ...
    // })
    this.shared.api.getStoresAll().subscribe((res: any) => {
      if (this.comm.pageName == this.SetTitle) {
        if (res.obj.storesData != undefined) {
          this.groupsArray = res.obj.storesData;
          this.stores = this.shared.common.groupsandstoresAll.filter((v: any) => v.sg_id == this.groupId)[0].Stores;
  ...
  openMainStoresDetails(ObjectOne: any, StoreName: any, Refer: any) {
    ...
    const DetailsET = this.ngbmodal.open(EnterpriseincomebyexpensewithoutoverheadDetails, {
      size: 'xl',
      backdrop: 'static',
